...
const PhotosCard = () => {
	...
	return (
		<DashboardCard title="Photos" link={photos && photos.length > 0 ? '/photos' : undefined} ariaLabel='View all your photos'>
				...
				{error ? (
					...
					<div className="grid grid-cols-2 grid-rows-2 gap-4 mx-auto w-fit">
						...
						{photos.length < 4 && Array.from({ length: 4 - photos.length }).map((_, index) => (
							<div
								key={index}
								className="relative overflow-hidden h-[90px] w-full rounded-lg border-yellow-300 border-2 bg-gray-300/80"
								aria-label="Empty photo slot"
							/>
						))}
					</div>
				) : (
					<div className="h-[200px]" aria-live="polite">
