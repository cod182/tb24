...
const AddTask = ({ handleSubmit, setTask, task }: { handleSubmit: (e: React.FormEvent) => void; setTask: React.Dispatch<React.SetStateAction<TaskProps>>, task: TaskProps }) => {
	...
	return (
		...
		<form onSubmit={handleSubmit} className="flex flex-col items-center justify-start w-full h-full gap-2 mt-2 grow sm:mt-0" >
			...
			<input id='title' onChange={(e) => setTask({ ...task, title: e.target.value })} className="w-full h-fit py-2 px-4 text-start text-black font-semibold rounded-lg z-[4]" placeholder="Enter Task Title" />

			<textarea id='descriptions' onChange={(e) => setTask({ ...task, description: e.target.value })} className="resize-none w-full h-fit sm:py-2 px-4 text-start text-black font-semibold rounded-lg z-[4] overflow-scroll" placeholder="Enter Task Description (Optional)" />
